...
- **映射(observability,`rateLimit`)**:`parseRateLimit` 在**每筆回應**(含 429 錯誤)解析整組 unified header → `RateLimitStatus`(`representative` + 代表 `status`/`resetAt` + `fiveHour`/`sevenDay` 各自 `{ utilization, resetAt, status }`;reset 秒→ms)。成功路徑掛 `GenerateResult.rateLimit` 與 stream `finish`;錯誤路徑也覆蓋寫入 `AnthropicProvider.rateLimit`(訂閱 429 若 header 未標 `rejected`/`exhausted` 會補 `status: "exhausted"`,僅有 reset 時寫 minimal 快照)。`status` 原則原樣透傳,不解讀門檻。**純觀測**,pool 不據此避讓(見 [pool.md](./pool.md));無 header 且非訂閱 429 → `undefined`。`clearRateLimit()` 丟棄 last-known 快照(管理端 `pool.clearMemberLimits` 會呼叫)。
...
用 **SuperGrok 訂閱額度**(而非按量計費 API)跑推論,對應 Anthropic 的訂閱 OAuth 路徑。`grok login` 會把 token 寫進 `~/.grok/auth.json`(細節對照開源 `xai-org/grok-build` 校正,詳見 `assets/gb/extracted/auth.md`)。**已對 live proxy 端到端驗證。**
...
- **auth 解析**:`~/.grok/auth.json` 結構 `{ "<scope>": { "key": <accessToken>, "refresh_token", "expires_at"(ISO), "oidc_issuer", "oidc_client_id", "auth_mode" } }`,scope 為 `{issuer}::{client_id}`;`GROK_AUTH_SCOPE` 常數即預設 scope key。同檔可並存多個 scope,`detect` 會跳過 `xai::api_key`(按量計費 key)與 `web_login`(已停用)並優先挑帶 refresh token 的 session,同時讀出 `oidc_issuer`/`oidc_client_id`。provider 本身 **fetch-only**——token 由呼叫端傳入 `credentials`。長跑服務用 `loadLocalOAuth` / `localOAuthPool` 讀檔並掛 `onRefresh` 寫回(見 README Local OAuth)。
- **refresh**:走 OIDC discovery(`{issuer}/.well-known/openid-configuration` → `token_endpoint`;預設 issuer `https://auth.x.ai`,可傳 `issuer`/`tokenUrl` 覆寫)後,對該 endpoint 送 **form-encoded** `refresh_token` grant,`client_id` 預設公開 client `b1a00492-…`(或憑證檔的 `oidc_client_id`),team 登入再帶 `principal_type`/`principal_id`;IdP 未輪替時保留舊 refresh token。到期前(`expiresAt`)主動換、401 再換一次;共用單一 round-trip;`onRefresh` 回寫;`invalid_grant` 等 OAuth2 `error` code 併入 auth 錯誤訊息。舊版曾誤打 `accounts.x.ai`(consent 網頁)+ `client_id=grok-cli`,故回 HTML 非 JSON;現已修正。bare token(無 refresh_token)視為 inference-only,到期於 401 清楚報錯。
